...
Deno.serve(async req => {
  ...
  try {
    ...
    const admin = adminClient(), existing = await admin.from('bank_connections').select('id,status').eq('user_id', user.id).eq('institution_id', bankKey).in('status', ['pending', 'linked']).maybeSingle()
